...
class CRF_word2vec(object):
    ...
    # def train_CRF(self,vali_file_result):
    #     # 设定模型训练的一些参数
    #     m_num = 5  # 迭代次数
    #     p_num = 20  # 线程数
    #     f_num = 3  # 属性出现次数不少于
    #     c_num = 4.0  # 最低损失值
    #
    #     os.system('crf_learn -a MIRA -m' + str(m_num) + ' -p ' + str(p_num) + ' -f' + str(f_num) + ' -c' + str(
    #         c_num) + ' template ' + self.vec_file + ' w2v_model')
    #     print "ＣＲＦ模型训练完成"
    #
    #     os.system('crf_test -m w2v_model  ' + self.vali_file + '>' + vali_file_result)
    #     print "CRF验证文档已经完成"
...
if __name__ == '__main__':
    ...
    #训练集 输入文档 和 转换格式后的文档
    p1 = text_class2("training_bao.txt", "lab_tag.txt")
    p1.get_text1()
    p3 = CRF_word2vec(w2v_train_file, vec_num, min_count, vec_choice, vali_file,p1.out_put)
    p3.train_document()

    # 验证集
    p2 = text_class2("vali_bao.txt", "lab_tag2.txt")
    p2.get_text1()
    p4 = CRF_word2vec(w2v_train_file, vec_num, min_count, vec_choice, vali_file, p2.out_put)
    p4.vali_document()

    # #通过ＣＲＦ模型生成最终验证的文档
    # vali_file_result = "3vali_result.txt"
    # p3.train_CRF(vali_file_result)
    #
    # #输出最终的准确率
    # v = get_reuslt_goal(vali_file_result)
    # v.result_goal()
